...
import { Bell, X, Check, CheckCheck, AlertCircle, Clock, UserPlus, UserMinus, Calendar, FileText, LogIn, LogOut, Server, AlertTriangle } from 'lucide-react';
...
export default function NotificationBell() {
  ...
  const fetchNotifications = async () => {
    try {
      setLoading(true);
      const res = await fetch('/app/api/admin/notifications?limit=20');
  ...
  const setupSSE = () => {
    try {
      const eventSource = new EventSource('/app/api/admin/notifications/stream');
          ...
          {notifications.length > 0 && (
            <div className="p-3 border-t border-slate-200 dark:border-slate-800">
              <button
                ...
                className="w-full text-sm text-center text-[#E39A2E] hover:text-[#d18a1f] font-medium py-2 cursor-pointer"
              >
